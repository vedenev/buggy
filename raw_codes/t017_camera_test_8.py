...
POINTS_GAP_SIZE_RELATIVE = 0.15
POINTS_GAP_SIZE_RELATIVE_MIN = 0.02
        ...
        for point_count_1 in range(0, points.shape[0] - 1):
            ...
            for point_count_2 in range(point_count_1 + 1, points.shape[0]):
                ...
                valid_points[point_count_1] and valid_points[point_count_2]:
                    ...
                    code_binary = np.zeros(code_binary_length, np.bool)
                    code_binary[0] = point_1_sign > 0
                    code_binary[1] = point_2_sign > 0
                    code_binary[2: 2 + CODE_SHAPE[1] - 4] = image_rotated_tmp0[0, 1:-1] < threshold_black_white
                    code_binary[2 + CODE_SHAPE[1] - 4:] = image_rotated_tmp0[1, 1:-1] < threshold_black_white
                    
                    signal_strengh = np.mean(np.abs(image_rotated_tmp0[:, 1:-1] - threshold_black_white))
                    
                    if signal_strengh > 30.0:
