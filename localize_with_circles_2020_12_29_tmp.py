...
#PIXELS_TO_ANGLE = np.load('PIXELS_TO_ANGLE_22.npy')
#PIXELS_TO_ANGLES_TAN_X = np.load('PIXELS_TO_ANGLES_TAN_X_22.npy')
#PIXELS_TO_ANGLES_TAN_Y = np.load('PIXELS_TO_ANGLES_TAN_Y_22.npy')
...
x_measured = np.asarray([   4,   15,   26,   38,   98,  165,  235,  307,  377,  445,  508,
        565,  617,  664,  708,  752,  799,  851,  908,  971, 1038, 1109,
       1179, 1234, 1248, 1261, 1274], dtype=np.float32)

angles_measured = np.asarray([-1.2082521 , -1.1872689 , -1.1659045 , -1.1441689 , -1.0303769 ,
       -0.9097532 , -0.7853982 , -0.66104317, -0.5404195 , -0.42662752,
       -0.32175058, -0.22679889, -0.14189708, -0.0665682 ,  0.        ,
        0.0665682 ,  0.14189708,  0.22679889,  0.32175058,  0.42662752,
        0.5404195 ,  0.66104317,  0.7853982 ,  0.88506687,  0.9097532 ,
        0.93428814,  0.95864385], dtype=np.float32)
from scipy.interpolate import interp1d
f_measured = interp1d(x_measured, angles_measured, kind='linear', fill_value='extrapolate', assume_sorted=True)
def pixels_to_angle_from_measured_linear(x, y):
    global f_measured
    angles = f_measured(x)
    return angles
...
def localize_with_circles_tmp(x_detected_all, y_detected_all, code_detected_all, code_size_all, x_detected_pairs_all, y_detected_pairs_all):
    ...
    if x_detected_all.size >= 2: # must be at least two codes
        ...
        #angles_detected_pairs_all = pixels_to_angle_from_measured(x_detected_pairs_all, y_detected_pairs_all)
        print("angles_detected_pairs_all =", angles_detected_pairs_all)
        ...
        for code_index in range(code_detected_all.size):
            ...
            print("angles_deltas_tmp_tmp =", angles_deltas_tmp_tmp)
        ...
        for circle_index in range(circles.shape[0]):
            ...
            plt.plot(x_circle + R_tmp * np.cos(a_t), y_circle + R_tmp * np.sin(a_t), markers_tmp[marker_counter_tmp % len(markers_tmp)])
        marker_counter_tmp += 1
        ...
        detected_pairs_tmp2 = np.zeros((n_circle_pairs, 2), np.int64)
        global_index = 0
        for circle_index_1 in range(circles.shape[0] - 1):
            for circle_index_2 in range(circle_index_1 + 1, circles.shape[0]):
                ...
                #print(code_detected_all[circle_index_1], code_detected_all[circle_index_2], "simularity =", simularity, " one_solusness =", one_solusness)
                ...
                if is_pair_ok_all[global_index]:
                    ...
                    print('consistency_c1 =', consistency_c1)
                    print('consistency_c2 =', consistency_c2)
                    ...
                    detected_pairs_tmp2[global_index, 0] = code_detected_all[circle_index_1]
                    detected_pairs_tmp2[global_index, 1] = code_detected_all[circle_index_2]
        ...
        if is_pair_ok_indexes_all.size > 0:
            ...
            print("is_pair_ok_indexes_all =", is_pair_ok_indexes_all)
            print("x_cross_all =", x_cross_all)
            print("y_cross_all =", y_cross_all)
            print("gamma_all =", gamma_all)
            print("detected_pairs_tmp2[is_pair_ok_indexes_all, :] =", detected_pairs_tmp2[is_pair_ok_indexes_all, :])
