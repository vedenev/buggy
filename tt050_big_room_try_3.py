...
MINIMAL_MOVABLE_VALUE_RIGHT = 405
MINIMAL_MOVABLE_VALUE_LEFT = 405


ANGLE_SMOOTH_CONTROL_THRESHOLD = np.pi / 4
SMOOTH_CONTROL_BASE_VALUE = 405
SMOOTH_CONTROL_KP = 75.0
...
for frame_0 in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    is_localized_by_odometry = False;
    counter_right_tmp = counter_right
    counter_left_tmp = counter_left
    if is_localized_tmp:
        is_localized_once = True
        counter_right_old = counter_right_tmp
        counter_left_old = counter_left_tmp
    ...
    counter_right_all = np.append(counter_right_all, counter_right_tmp)
    counter_left_all = np.append(counter_left_all, counter_left_tmp)
