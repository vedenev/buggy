    ...
    for iterations_index in range(N_SOLVE_ITERATIONS):
        ...
        for detected_index in range(angles_detected_pairs_all.shape[0]):
            ...
            for in_pair_index in range(2):
                ...
                right_side[global_index] = -((x_real - current_solution[0, 0]) * sin_ray_angle - (y_real - current_solution[1, 0]) * cos_ray_angle)
                dot_products[global_index] = (x_real - current_solution[0, 0]) * cos_ray_angle + (y_real - current_solution[1, 0]) * sin_ray_angle
                jacobain[global_index, 0] = -sin_ray_angle
                jacobain[global_index, 1] = cos_ray_angle
                jacobain[global_index, 2] = (x_real - current_solution[0, 0]) * cos_ray_angle + (y_real - current_solution[1, 0]) * sin_ray_angle
