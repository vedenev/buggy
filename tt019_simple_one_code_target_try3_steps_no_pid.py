...
STRIGHT_MOTION_ANGLE_RELATIVE = 0.1
stright_motion_angle_pixels = int(WIDTH * STRIGHT_MOTION_ANGLE_RELATIVE)
time_tmp = 0.0
    ...
    if mode == 0:
        if measure_current_time > 0.0:
            ...
            if was_datection_at_measure:
                ...
                delta_tmp = np.abs(detection_at_measure_x - (WIDTH  / 2))
                if delta_tmp <= stright_motion_angle_pixels:
                    right_value = MINIMAL_MOVABLE_VALUE_STRIGHT_RIGHT
                    left_value = MINIMAL_MOVABLE_VALUE_STRIGHT_LEFT
                else:
